/**
 * r51 — OPTION B PRODUCTION VALIDATION AFTER R50 (mizd00320, max 1 LLM).
 *
 * Observation only. No prompt/brain/schema/rule/code fixes this round.
 *
 *   OUT_DIR=/tmp/prod-gen-20260821-r51-mizd00320 \
 *   BLOG_MAX_PLAN_ATTEMPTS=1 \
 *   BLOGGER_DRAFT_ID=5758756966895197750 \
 *   npx tsx src/ops/r51-option-b-production-validation.ts
 */
...
const OUT =
  process.env.OUT_DIR ||
  `/tmp/prod-gen-${new Date().toISOString().slice(0, 10).replace(/-/g, "")}-r51-mizd00320`;
...
class SingleCallLLMProvider implements LLMProvider {
  ...
  async executeTask(request: LLMTaskRequest): Promise<LLMTaskResult> {
    if (this.used) {
      throw new Error("r51 HARD LIMIT: second LLM call blocked (max 1)");
    }
    this.used = true;
...
async function main() {
  ...
  // r50 CORRECT: Writer projection must not present equivalent durations as independent inputs
  {
    const rawOfficial =
      typeof officialDescription === "string" ? officialDescription : "";
    const projDesc = String(writerSourceMaterial.officialDescription ?? "");
    const titleHas8 = /8\s*時間/.test(product.title);
    const rawHas480 = /480\s*分/.test(rawOfficial);
    const projHas480 = /480\s*分/.test(projDesc);
    const projHas8 = /8\s*時間/.test(projDesc);
    if (titleHas8 && rawHas480 && projHas480) {
      mismatches.push("duration_writer_projection_still_dual_independent:480分_retained");
    }
    if (titleHas8 && rawHas480 && !projHas8) {
      mismatches.push("duration_writer_projection_did_not_collapse_to_preferred");
    }
  }
    ...
    JSON.stringify(
      {
        ...
        writerProjectedHas480分: /480\s*分/.test(String(writerSourceMaterial.officialDescription ?? "")),
        writerProjectedHas8時間: /8\s*時間/.test(String(writerSourceMaterial.officialDescription ?? "")),
        rawOfficialHas480分: /480\s*分/.test(String(officialDescription ?? "")),
  ...
  if (
    ...
    try {
      ...
      const prepared = await publisher.prepare({
        contentVersionId: versionId ?? `r51-human-review:${sample.label}`,
        title: postDefault.title,
        body: html,
  ...
  const report = {
    ...
    D_duration: {
      writerInputSurfaces: {
        productTitleMatches: String(writerSourceMaterial.productTitle ?? "").match(/\d+\s*(?:時間|分)/g) ?? [],
        officialDescriptionMatches:
          String(writerSourceMaterial.officialDescription ?? "").match(/\d+\s*(?:時間|分)/g) ?? [],
        dualIndependent480With8h:
          /8\s*時間/.test(String(writerSourceMaterial.productTitle ?? "")) &&
          /480\s*分/.test(String(writerSourceMaterial.officialDescription ?? "")),
      },
      rawDurationExpressions: {
        matches: JSON.stringify(providerRaw).match(/\d+\s*(?:時間|分)/g) ?? [],
        has_8時間: /8\s*時間/.test(JSON.stringify(providerRaw)),
        has_480分: /480\s*分/.test(JSON.stringify(providerRaw)),
        parentheticalBoth:
          /8\s*時間\s*[（(]\s*480\s*分\s*[）)]/.test(JSON.stringify(providerRaw)) ||
          /480\s*分\s*[（(]\s*8\s*時間\s*[）)]/.test(JSON.stringify(providerRaw)),
      },
      quantityObs,
    },
    E_schema: {
    ...
    I_Brain: {
      ...
      evaluativeInference: findingCodes.includes("EVALUATIVE_INFERENCE"),
      socialProof: findingCodes.includes("SOCIAL_PROOF"),
      repetition: findingCodes.includes("REPETITION"),
      filler: findingCodes.includes("FILLER") || findingCodes.includes("FILLER_META"),
      informationDensityLow: findingCodes.includes("INFORMATION_DENSITY_LOW"),
    },
    J_r50CorrectionVerification: (() => {
      const writerDual =
        /8\s*時間/.test(String(writerSourceMaterial.productTitle ?? "")) &&
        /480\s*分/.test(String(writerSourceMaterial.officialDescription ?? ""));
      const codes = findingCodes as string[];
      const rawText = JSON.stringify(providerRaw);
      const hasCharmParaphrase = /魅力|楽しめ/.test(rawText);
      const hasPopular = /人気/.test(rawText);
      return {
        DURATION: {
          result: writerDual ? "FAIL" : "PASS",
          reason: writerDual
            ? "Writer projection still exposes 8時間 and 480分 as independent inputs"
            : "Writer projection does not dual-feed equivalent duration surfaces",
        },
        SOURCE_SUPPORTED_EVALUATIVE: {
          result: codes.includes("EVALUATIVE_INFERENCE") ? "FAIL" : "PASS",
          reason: codes.includes("EVALUATIVE_INFERENCE")
            ? "EVALUATIVE_INFERENCE present in Brain findings (inspect if source-supported)"
            : "No EVALUATIVE_INFERENCE BLOCKING",
          charmOrEnjoyObserved: hasCharmParaphrase,
        },
        GENERAL_POPULARITY: {
          result: !hasPopular
            ? "NOT_OBSERVED"
            : codes.includes("SOCIAL_PROOF")
              ? "FAIL"
              : "PASS",
          reason: !hasPopular
            ? "人気 not present in RAW"
            : codes.includes("SOCIAL_PROOF")
              ? "人気 present and SOCIAL_PROOF finding fired (inspect whether fabricated proof)"
              : "人気 present without SOCIAL_PROOF BLOCKING",
        },
      };
    })(),
    K_humanDisplayFull: humanDisplay,
    L_images: {
    ...
    N_human: "PENDING_HUMAN_REVIEW",
    O_BLOG_QUALITY_STATUS: "PENDING_HUMAN_REVIEW",
    P_vs_r48: {
      note: "filled post-hoc in HUMAN_REPORT comparison helper; see RESULT vs r48 artifacts",
    },
    Q_llmCalls: llm.captured.result ? 1 : 0,
    R_regen: 0,
    S_repair: 0,
    T_automaticProseMutation: 0,
    modelRunId,
